...
| 推理延迟 | p50 31.8 ms，p95 33.2 ms，max 47.2 ms（816 帧，Console 模式）|
...
深度结构已在有真实纵深的房间场景中核对：近处家具一侧为红，地面延伸与
远墙为蓝，`zones` 数值与 PiP 画出的颜色一致。正对天花板这类大面积无纹理
场景（文档记载的失效条件）下可用范围会塌缩（那里 p98-p02 为 0.41，房间
场景为 1.72），所以评估这个模型要用物体距离差异明显的场景。
