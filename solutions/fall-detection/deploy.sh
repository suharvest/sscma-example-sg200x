#!/bin/bash
# deploy.sh - Deploy fall-detection to a reCamera device
...
if ! run_ssh "[ -f $POSE_MODEL ]"; then
    err "Pose model missing on device: $POSE_MODEL
     ...
     cvimodel there and point MODEL_PATH at it in /etc/fall-detection.conf."
fi
...
run_sudo 'killall -q fall-detection ppocr-reader face-analysis detection-blur retail-vision facemesh-reader weather-classifier yolo-detector sscma-node 2>/dev/null || true'
sleep 2
ok "Services stopped"
...
if [ "$DO_START" = false ]; then
    ...
    echo ""
    echo "   Not started. Open the console and enable \"Fall Detection\" —"
