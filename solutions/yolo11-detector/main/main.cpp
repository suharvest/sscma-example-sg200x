...
using namespace ma;
using namespace yolo11;

#define TAG "yolo11-detector"
...
static struct {
    ...
    int mqtt_port = 1883;
    std::string mqtt_topic = "recamera/yolo11/detections";

    // Video configuration
    // Use 16:9 aspect ratio to match RTSP stream FOV, letterbox to 640x640 for model
...
static Yolo11Detector* g_detector = nullptr;
...
static void print_usage(const char* prog) {
    printf("YOLO11 Object Detector for ReCamera\n");
...
static bool init_detector() {
    g_detector = new Yolo11Detector();
    if (!g_detector->init(g_config.model_path)) {
        MA_LOGE(TAG, "Failed to initialize YOLO11 detector");
        return false;
    }
    ...
    MA_LOGI(TAG, "YOLO11 detector initialized (input: %dx%d)",
            ...
            if (g_config.verbose || !detections.empty()) {
                ...
                for (const auto& det : detections) {
                    MA_LOGI(TAG, "  [%d] %s (%.1f%%) at (%.2f, %.2f, %.2f, %.2f)",
                            det.id,
                            Yolo11Detector::getClassName(det.class_id),
...
int main(int argc, char** argv) {
    ...
    MA_LOGI(TAG, "Starting YOLO11 Object Detector");
    ...
    MA_LOGI(TAG, "YOLO11 detector running...");
    ...
    MA_LOGI(TAG, "YOLO11 Object Detector terminated");
    return 0;
}
