...
constexpr int Yolo11Detector::GRID_SIZES[3];
constexpr int Yolo11Detector::STRIDES[3];
...
bool Yolo11Detector::init(const std::string& model_path) {
    MA_LOGI(TAG, "Initializing YOLO11 detector with model: %s", model_path.c_str());
    ...
    // Verify we have 6 outputs (bbox + cls for 3 scales)
    ...
    MA_LOGI(TAG, "YOLO11 detector initialized");
...
void Yolo11Detector::setConfThreshold(float threshold) {
    conf_threshold_ = std::max(0.0f, std::min(1.0f, threshold));
}

void Yolo11Detector::setNmsThreshold(float threshold) {
    nms_threshold_ = std::max(0.0f, std::min(1.0f, threshold));
}

const char* Yolo11Detector::getClassName(int class_id) {
...
float Yolo11Detector::computeDFL(const float* bins) const {
    // Softmax over DFL_LEN bins, then weighted sum
...
void Yolo11Detector::letterboxPreprocess(const ma_img_t* src) {
...
void Yolo11Detector::transformCoordinates(std::vector<Detection>& detections) {
...
std::vector<Detection> Yolo11Detector::detect(ma_img_t* img) {
...
void Yolo11Detector::decodeOutputs(std::vector<Detection>& results) {
    results.clear();

    /**
     * YOLO11 output format (6 outputs):
     *   - bbox tensors: shape 1×64×H×W (4 coords × 16 DFL bins)
     *   - cls tensors: shape 1×80×H×W (class scores)
     *
     * DFL decoding for each grid cell:
     *   1. Extract 64 values (4 × 16 bins) from bbox tensor
     *   2. For each of 4 coordinates: softmax(16 bins) → weighted sum = distance
     *   3. Distances: left, top, right, bottom (from grid center)
     *   4. Convert: x1 = (grid_x + 0.5 - left) * stride
     *              x2 = (grid_x + 0.5 + right) * stride
     *              y1 = (grid_y + 0.5 - top) * stride
     *              y2 = (grid_y + 0.5 + bottom) * stride
     */

    // Pre-compute threshold in logit space
    ...
    for (int i = 0; i < NUM_OUTPUTS; i++) {
        ...
        if (scale_idx >= 0) {
            if (c == BBOX_CHANNELS) {  // 64 channels = DFL bbox
    ...
    for (int scale = 0; scale < 3; scale++) {
        ...
        for (int j = 0; j < grid_size; j++) {
            for (int k = 0; k < grid_size; k++) {
                ...
                // DFL decode: extract 64 values and compute 4 distances
                float dfl_bins[DFL_LEN];
                float distances[4];  // left, top, right, bottom

                for (int coord = 0; coord < 4; coord++) {
                    // Extract 16 bins for this coordinate
                    if (is_int8) {
                ...
                // Convert DFL distances to box coordinates
                // distances: [left, top, right, bottom] from grid center
...
void Yolo11Detector::applyNMS(std::vector<Detection>& detections) {
    if (detections.empty()) {
        return;
    }
