#!/bin/bash
# deploy.sh - Deploy fitness-trainer to a reCamera device
# Handles: build -> stop camera-using services -> install deb -> start -> verify
#
# Usage:
#   ./deploy.sh                          # Build + deploy
#   ./deploy.sh --skip-build             # Deploy the existing deb only
#   ./deploy.sh --host 10.0.0.1          # Custom host
#   ./deploy.sh --mode push_up           # Set the exercise while deploying
...
#     SECOND instance of whatever it still thinks is active. That race is
#     what produced "failed to start debug stream on port 8001" here: two
#     processes, one port, and a live-looking app with no debug stream.
#   * Its stop path waits for the VPSS group to be released before starting
#     anything else (wedge-defense B) instead of sleeping and hoping.
#
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            echo "Usage: $0 [--host IP] [--user USER] [--password PASS] [--mode squat|push_up|hammer_curl] [--skip-build] [--start] [--no-mqtt]"
...
if [ -n "$MODE" ]; then
    case "$MODE" in
        squat|push_up|hammer_curl) ;;
        *) err "Unknown --mode '$MODE' (squat | push_up | hammer_curl)" ;;
    esac
fi
...
# --- Step 2: Check the pose model is on the device ---
# It ships with the supervisor rather than with this app, so a device running an
# older supervisor has no model and the app would die on start.
POSE_MODEL=/usr/share/supervisor/models/yolo11n_pose_cv181x_int8.cvimodel
if ! run_ssh "[ -f $POSE_MODEL ]"; then
    err "Pose model missing on device: $POSE_MODEL
     It ships with the supervisor. Update the supervisor, or copy a YOLO pose
     cvimodel there and point MODEL_PATH at it in /etc/fitness-trainer.conf."
fi
ok "Pose model present"
...
run_ssh 'for svc in /etc/init.d/[SK]*sscma-node* /etc/init.d/[SK]*node-red* \
               ...
               /etc/init.d/[SK]*facemesh* /etc/init.d/[SK]*weather-classifier*; do
    [ -x "$svc" ] && "$svc" stop 2>/dev/null || true
done' || warn "Some init scripts not found (OK)"

run_sudo 'killall -q fitness-trainer ppocr-reader face-analysis detection-blur retail-vision facemesh-reader weather-classifier yolo-detector sscma-node 2>/dev/null || true'
sleep 2
ok "Services stopped"
...
# --- Step 5: Optional exercise selection ---
if [ -n "$MODE" ]; then
    log "Setting exercise to ${MODE}..."
    # Staged through a file rather than a remote redirect: `sudo cmd > path`
    # redirects in the *calling* shell, which is not root, so writing under
    # /userdata/local/apps that way fails with Permission denied.
    TMP_CFG="$(mktemp)"
    printf '{"mode":"%s"}\n' "$MODE" > "$TMP_CFG"
    run_scp "$TMP_CFG" "${USER}@${HOST}:/tmp/${SOLUTION_NAME}.config.json" || err "Config upload failed"
    rm -f "$TMP_CFG"
    run_sudo "install -D -m 644 /tmp/${SOLUTION_NAME}.config.json /userdata/local/apps/${SOLUTION_NAME}.config.json" \
        || err "Failed to install config"
    ok "Exercise set to ${MODE}"
fi
...
if [ "$DO_START" = false ]; then
    ...
    echo ""
    echo "   Not started. Open the console and enable \"Fitness Trainer\" —"
    ...
    echo "   Config:  /userdata/local/apps/${SOLUTION_NAME}.config.json"
    exit 0
fi
...
# Wedge-defense (B), replicated from the supervisor's sh_stop: wait for the
# camera's VPSS group to be handed back before starting the next owner.
# Starting while Grp(0) is still alive is what yields "Grp(0) is occupied" and
# then an endless "get chn frame fail". Parses the GRP ATTR table of
# /proc/cvitek/vpss, whose data rows start with '#'; with no owner the table
...
run_sudo "tail -n 20 /var/log/${SOLUTION_NAME}.log" || warn "No log yet"
