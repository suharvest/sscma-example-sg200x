#ifndef _FITNESS_POSE_H_
#define _FITNESS_POSE_H_

// COCO-17 keypoint semantics and the one geometric primitive every exercise
// needs (the angle at a joint).
//
...
//
// Note for anyone diffing against the original: its squat and hammer-curl
// trackers labelled MediaPipe 11/13/15 as "right" when they are the left side.
// Harmless there (it only mislabels the on-screen text), but do not carry the
// naming over.
...
public:
    ...
    // Used to pick the better-facing side of a two-sided exercise.
