...
**典型场景**：进出店客流统计支撑排班、停留热点发现优化陈列位、求助状态识别驱动导购响应。
...
- **隐私**：开启设备级打码后，检测到的人会在编码之前被遮挡，因此 RTSP 流、console 预览、`/snapshot.jpg` 拿到的都已经是打码后的画面。默认关闭，开关与透明度在 console 的**设备**页（调试页有快捷开关），改动即时生效、不重启应用。
