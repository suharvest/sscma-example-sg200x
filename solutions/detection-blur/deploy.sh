#!/bin/bash
# deploy.sh - Deploy detection-blur to reCamera device
# Handles: build → stop all camera services → install deb → start service → verify
#
# Usage:
...
run_ssh 'for svc in /etc/init.d/S*sscma-node* /etc/init.d/K*sscma-node* \
               ...
               /etc/init.d/S*detection-blur* /etc/init.d/K*detection-blur*; do
    [ -x "$svc" ] && "$svc" stop 2>/dev/null || true
done' || warn "Some init scripts not found (OK)"

run_sudo 'killall -q detection-blur face-analysis ppocr-reader yolo8-detector yolo11-detector yolo26-detector yolo11s-detector sscma-node 2>/dev/null || true'
sleep 2
ok "Services stopped"
...
run_sudo "/etc/init.d/S92detection-blur restart" || err "Service start failed"
ok "Service started"
...
run_ssh "/etc/init.d/S92detection-blur status" || warn "Status check failed"

if [ "$MQTT_CHECK" = true ]; then
    log "Capturing MQTT output (10s, max 3 messages)..."
    run_ssh "timeout 10 mosquitto_sub -h localhost -t 'recamera/detection-blur/results' -C 3" || warn "MQTT check timed out"
fi
