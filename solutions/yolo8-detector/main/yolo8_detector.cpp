...
constexpr int Yolo8Detector::GRID_SIZES[3];
constexpr int Yolo8Detector::STRIDES[3];

Yolo8Detector::Yolo8Detector()
    : engine_(nullptr),
...
bool Yolo8Detector::init(const std::string& model_path) {
    MA_LOGI(TAG, "Initializing YOLO8 detector with model: %s", model_path.c_str());
    ...
    MA_LOGI(TAG, "YOLO8 detector initialized");
...
void Yolo8Detector::setConfThreshold(float threshold) {
    conf_threshold_ = std::max(0.0f, std::min(1.0f, threshold));
}

void Yolo8Detector::setNmsThreshold(float threshold) {
    nms_threshold_ = std::max(0.0f, std::min(1.0f, threshold));
}

const char* Yolo8Detector::getClassName(int class_id) {
...
float Yolo8Detector::computeDFL(const float* bins) const {
...
void Yolo8Detector::letterboxPreprocess(const ma_img_t* src) {
...
void Yolo8Detector::transformCoordinates(std::vector<Detection>& detections) {
...
std::vector<Detection> Yolo8Detector::detect(ma_img_t* img) {
...
void Yolo8Detector::decodeOutputs(std::vector<Detection>& results) {
    results.clear();
...
void Yolo8Detector::applyNMS(std::vector<Detection>& detections) {
    if (detections.empty()) {
        return;
    }
