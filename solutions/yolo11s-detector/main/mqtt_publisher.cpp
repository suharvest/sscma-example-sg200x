...
bool MqttPublisher::init(const MqttConfig& config) {
    if (initialized_.load()) { MA_LOGW(TAG, "Already initialized"); return true; }

    config_ = config;
    mosquitto_lib_init();

    client_ = mosquitto_new(config_.client_id.c_str(), true, this);
    if (!client_) { MA_LOGE(TAG, "Failed to create MQTT client"); return false; }
...
void MqttPublisher::onConnect(int rc) {
    if (rc == 0) { connected_.store(true); MA_LOGI(TAG, "Connected to MQTT broker"); }
    else MA_LOGE(TAG, "Connection failed with code: %d", rc);
}

void MqttPublisher::onDisconnect(int rc) {
    connected_.store(false);
    if (rc != 0) MA_LOGW(TAG, "Unexpected disconnect: %d", rc);
    else MA_LOGI(TAG, "Disconnected from MQTT broker");
}

bool MqttPublisher::publish(const std::string& topic, const std::string& payload) {
    if (!initialized_.load() || !client_) return false;
    if (!connected_.load()) MA_LOGV(TAG, "Not connected, message will be queued");

    int rc = mosquitto_publish(client_, nullptr, topic.c_str(),
                                static_cast<int>(payload.size()),
                                payload.data(), config_.qos, config_.retain);
    if (rc != MOSQ_ERR_SUCCESS) { MA_LOGE(TAG, "Publish failed: %d", rc); return false; }
    return true;
}
...
std::string MqttPublisher::buildResultJson(uint64_t timestamp_ms, uint32_t frame_id,
    ...
    json << "\"frame_width\":1,\"frame_height\":1,";
    json << "\"detections\":[";
    ...
    for (size_t i = 0; i < detections.size(); ++i) {
        ...
        json << "\"class_name\":\"" << Yolo11sDetector::getClassName(det.class_id) << "\",";
...
bool MqttPublisher::publishResults(uint64_t timestamp_ms, uint32_t frame_id,
                                    const std::vector<Detection>& detections,
                                    float inference_time_ms) {
    return publish(config_.topic, buildResultJson(timestamp_ms, frame_id, detections, inference_time_ms));
}
...
bool MqttPublisher::publishTrackingResults(uint64_t timestamp_ms, uint32_t frame_id,
                                            ...
                                            const StateCount& counts,
                                            float inference_time_ms) {
    return publish(config_.topic, buildTrackingJson(timestamp_ms, frame_id, persons, counts, inference_time_ms));
}
