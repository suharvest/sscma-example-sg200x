...
float PersonTracker::computeIoU(const Detection& a, const Detection& b) const {
    ...
    float inter_x1 = std::max(ax1, bx1), inter_y1 = std::max(ay1, by1);
    float inter_x2 = std::min(ax2, bx2), inter_y2 = std::min(ay2, by2);
...
std::vector<std::pair<int, int>> PersonTracker::matchDetections(
    ...
    if (tracks_.empty() || detections.empty()) return matches;

    std::vector<int> track_ids;
    for (const auto& pair : tracks_) track_ids.push_back(pair.first);
    ...
    for (int track_id : track_ids) {
        ...
        for (size_t d = 0; d < detections.size(); d++) {
            ...
            if (iou > best_iou) { best_iou = iou; best_det_idx = static_cast<int>(d); }
        }
...
void PersonTracker::updateVelocity(TrackedPerson& track, const Detection& new_det, float dt) {
    if (dt <= 0.001f) return;

    float dx = (new_det.x - track.detection.x) * config_.frame_width;
    float dy = (new_det.y - track.detection.y) * config_.frame_height;
    float ivx = dx / dt, ivy = dy / dt;
    float instant_speed = std::hypot(ivx, ivy);

    float prev_speed = track.speed_px_s;
    bool sudden_stop = (prev_speed > 10.0f) && ((prev_speed - instant_speed) / prev_speed > 0.5f);

    if (instant_speed < config_.velocity_zero_threshold) {
        track.velocity_x = track.velocity_y = track.speed_px_s = 0.0f;
    } else {
    ...
    track.speed_normalized = (bbox_h_px > 1.0f) ? (track.speed_px_s / bbox_h_px) * 100.0f : 0.0f;
}
...
void PersonTracker::updateDwellState(TrackedPerson& track, float current_time) {
    ...
    if (track.stationary_frames < config_.dwell_min_frames) return;

    if (track.dwell_start_time <= 0.0f) track.dwell_start_time = current_time;
...
std::vector<TrackedPerson> PersonTracker::update(
    const std::vector<Detection>& detections, float current_time_sec
) {
    ...
    for (const auto& det : detections)
        if (det.class_id == 0) person_detections.push_back(det);
    ...
    for (auto& pair : tracks_) {
        ...
        if (std::find(matched_track_ids.begin(), matched_track_ids.end(), track_id) == matched_track_ids.end()) {
    ...
    for (const auto& pair : tracks_)
        if (pair.second.lost_frames == 0) result.push_back(pair.second);
...
StateCount PersonTracker::getStateCounts() const {
    ...
    for (const auto& pair : tracks_) {
        ...
        switch (track.dwell_state) {
            case DwellState::TRANSIENT: case DwellState::DWELLING: counts.browsing++; break;
            case DwellState::ENGAGED: counts.engaged++; break;
            case DwellState::ASSISTANCE: counts.assistance++; break;
        }
    }
    return counts;
}
