...
using namespace ma;
using namespace yolo11s;

#define TAG "yolo11s-detector"

// Default configuration
static struct {
    std::string model_path = "/userdata/local/models/yolo11s_detection_cv181x_int8.cvimodel";
    ...
    int mqtt_port = 1883;
    std::string mqtt_topic = "recamera/yolo11s/detections";
...
static Yolo11sDetector* g_detector = nullptr;
...
static void print_usage(const char* prog) {
    printf("YOLO11s Object Detector for ReCamera\n");
...
static bool init_video_streaming() {
    if (!g_config.enable_rtsp) { MA_LOGI(TAG, "RTSP streaming disabled"); return true; }

    if (initVideo() != 0) { MA_LOGE(TAG, "Failed to initialize video subsystem"); return false; }
...
static bool init_mqtt() {
    if (!g_config.enable_mqtt) { MA_LOGI(TAG, "MQTT publishing disabled"); return true; }
            ...
            if (g_config.verbose || !tracked_persons.empty()) {
                ...
                for (const auto& person : tracked_persons) {
                    MA_LOGI(TAG, "  [T%d] %s (%.1f%%) speed=%.1f px/s, dwell=%.1fs",
                            person.track_id, getDwellStateName(person.dwell_state),
                            person.detection.confidence * 100.0f,
                            person.speed_px_s, person.dwell_duration_sec);
                }
            }
        } else {
            ...
            if (g_config.verbose || !detections.empty()) {
                ...
                for (const auto& det : detections) {
                    MA_LOGI(TAG, "  [%d] %s (%.1f%%) at (%.2f, %.2f, %.2f, %.2f)",
                            det.id, Yolo11sDetector::getClassName(det.class_id),
                            det.confidence * 100.0f, det.x, det.y, det.w, det.h);
                }
            }
        }
    }
...
int main(int argc, char** argv) {
    ...
    MA_LOGI(TAG, "Starting YOLO11s Object Detector");
    ...
    MA_LOGI(TAG, "YOLO11s detector running...");
    MA_LOGI(TAG, "RTSP stream: rtsp://<device_ip>:8554/live0");
    MA_LOGI(TAG, "MQTT topic: %s", g_config.mqtt_topic.c_str());
    MA_LOGI(TAG, "Person tracking: %s", g_config.enable_tracking ? "ENABLED" : "DISABLED");

    while (g_running.load()) { process_frame(); }

    cleanup();
    MA_LOGI(TAG, "YOLO11s Object Detector terminated");
    return 0;
}
