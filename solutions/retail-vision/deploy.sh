#!/bin/bash
# deploy.sh - Deploy retail-vision to reCamera device
# Usage:
...
run_sudo 'killall -q retail-vision detection-blur face-analysis ppocr-reader yolo8-detector yolo11-detector yolo26-detector yolo11s-detector sscma-node 2>/dev/null || true'
sleep 2
ok "Services stopped"
...
# The package installs the init script K-prefixed (disabled at boot) on
# gallery devices, where sscma-supervisor is the sole boot-time orchestrator.
# Hard-coding S92 here made every successful deploy end in "Service start
# failed" — resolve whichever prefix is actually installed.
log "Starting ${SOLUTION_NAME}..."
run_sudo "sh -c 'set -e; f=\$(ls /etc/init.d/[SK]92retail-vision 2>/dev/null | head -1); [ -n \"\$f\" ] || { echo \"no retail-vision init script in /etc/init.d\" >&2; exit 1; }; \"\$f\" restart'" || err "Service start failed"
ok "Service started"
...
run_ssh "sh -c 'f=\$(ls /etc/init.d/[SK]92retail-vision 2>/dev/null | head -1); [ -n \"\$f\" ] && \"\$f\" status'" || warn "Status check failed"

if [ "$MQTT_CHECK" = true ]; then
    log "Capturing MQTT output (10s, max 3 messages)..."
    run_ssh "timeout 10 mosquitto_sub -h localhost -t 'recamera/retail-vision/#' -C 3" || warn "MQTT check timed out"
fi
